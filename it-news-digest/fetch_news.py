#!/usr/bin/env python3
"""Daily IT News Digest — fetches RSS feeds, summarizes with Gemini, generates HTML."""
...
NEWS_SOURCES = [
    {"name": "TechCrunch",          "url": "https://techcrunch.com/feed/",                              "category": "IT ทั่วไป"},
    {"name": "The Verge",           "url": "https://www.theverge.com/rss/index.xml",                    "category": "IT ทั่วไป"},
    {"name": "Ars Technica",        "url": "https://feeds.arstechnica.com/arstechnica/technology-lab",  "category": "IT ทั่วไป"},
    {"name": "MIT Tech Review",     "url": "https://www.technologyreview.com/feed/",                    "category": "AI / ML"},
    {"name": "Hacker News",         "url": "https://news.ycombinator.com/rss",                          "category": "Developer"},
]
...
def md_to_html_paragraphs(md: str) -> str:
    """Convert minimal Markdown (##, ###, bullet) to HTML."""
    import re
    lines = md.splitlines()
    out = []
    for line in lines:
...
def generate_html(articles: list[dict], summary_md: str, now: datetime) -> str:
    date_th = now.strftime("%-d %B %Y").replace(
    ...
    summary_html = md_to_html_paragraphs(summary_md)
    ...
    for cat, items in categories.items():
        cards_html += f'<section class="category"><h2 class="cat-title">{html_lib.escape(cat)}</h2><div class="cards">'
        for a in items:
            cards_html += f"""
            <article class="card">
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .date {{ color: var(--muted); margin-top: 0.4rem; font-size: 0.9rem; }}
    ...
    .summary-box h2 {{ font-size: 1.05rem; color: var(--accent2); margin: 1rem 0 0.3rem; }}
    .summary-box h3 {{ font-size: 0.95rem; color: var(--text); margin: 0.8rem 0 0.2rem; }}
    .summary-box p {{ color: var(--muted); font-size: 0.9rem; }}
    .summary-box li {{ color: var(--muted); font-size: 0.9rem; margin-left: 1.2rem; }}
    ...
    .card h3 {{ font-size: 0.95rem; font-weight: 600; line-height: 1.4; margin-bottom: 0.5rem; }}
    .card h3 a {{ color: var(--text); }}
    .card h3 a:hover {{ color: var(--accent); }}
    .snippet {{ font-size: 0.82rem; color: var(--muted); display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }}
...
<body>
  ...
  <footer>สร้างอัตโนมัติโดย GitHub Actions · แหล่งข่าว: TechCrunch, The Verge, Ars Technica, MIT Tech Review, Hacker News</footer>
</body>
</html>"""
